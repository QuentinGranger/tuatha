...
const navItems = [
  ...
  { label: "Programmes", href: "/dashboard/coach/programmes", icon: "clipboard" },
  { label: "Indicateurs de Performance", href: "/dashboard/coach/indicateurs", icon: "trending" },
  { label: "Réseau Professionnel", href: "/dashboard/coach/reseau", icon: "network" },
  { label: "Messagerie", href: "/dashboard/coach/messagerie", icon: "message" },
...
function getBreadcrumb(pathname: string): { label: string; href: string | null }[] {
  ...
  if (item && item.href !== "/dashboard/coach") {
    crumbs.push({ label: item.label, href: null });
  } else if (pathname.startsWith("/dashboard/coach/programmes/")) {
    crumbs.push({ label: "Programmes", href: "/dashboard/coach/programmes" });
          ...
          <div className={styles.topbarRight}>
            <div className={styles.notifWrapper} ref={notifRef}>
              ...
              {notifOpen && (() => {
                ...
                const levelColors = { urgent: "#ef4444", action: "#f97316", info: "#64748b" };
                const levelIcons = {
                        ...
                        return (
                          <div key={n.id} className={`${styles.notifItem} ${styles[`notifItem_${level}`]} ${readIds.has(n.id) ? styles.notifItemRead : ""}`} onClick={() => {
                            ...
                            <div className={styles.notifItemAction}>
                              {n.source === "visio_join" && n.meta?.roomId ? (
                                <button className={`${styles.notifActionBtn} ${styles.notifActionGreen}`} onClick={(e) => { e.stopPropagation(); openVisioRoom(n.meta.roomId); markAsRead(n.id); }}>Rejoindre</button>
                              ) : n.source === "invite" ? (
            ...
            <div className={styles.userInfo} ref={dropdownRef}>
              ...
              {dropdownOpen && (
                <div className={styles.dropdown}>
                  ...
                  <Link href="/dashboard/coach/profil" className={styles.dropdownItem} onClick={() => setDropdownOpen(false)}>
