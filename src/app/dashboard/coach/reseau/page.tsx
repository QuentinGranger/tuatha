...
const NOTE_TAGS = ["Blessure", "Charge", "Nutrition", "Retour séance", "Suivi", "Alerte"];
...
function PatientOverview({ onSelectAthlete }: { onSelectAthlete: (id: string) => void }) {
  ...
  return (
    <div className={styles.patientOverview}>
      ...
      {filtered.length === 0 && (
        <div className={styles.emptySection}>Aucun patient trouvé</div>
      )}
      ...
      <div className={styles.patientGrid}>
        {filtered.map((p: any) => (
          <div key={p.id} className={styles.patientCard} onClick={() => onSelectAthlete(p.id)}>
            <div className={styles.patientCardTop}>
              ...
              <div className={styles.patientIndicators}>
                ...
                {p.trend && trendIcons[p.trend] && (
                  <span className={styles.patientTrend}>{trendIcons[p.trend]}</span>
                )}
              </div>
            </div>
            ...
            {(!p.connectedPros || p.connectedPros.length === 0) && (
              <div className={styles.patientNoPro}>Aucun professionnel lié</div>
            )}
          </div>
        ))}
      </div>
        ...
        <div className={styles.headerControls}>
          ...
          {athleteId && (
            <div className={styles.headerBtns}>
              ...
              <button className={styles.btnSecondary} onClick={() => router.push(`/dashboard/coach/programmes?athleteId=${athleteId}`)}>Voir le programme</button>
            </div>
          )}
        </div>
      </div>
        ...
        <div className={styles.disclaimer}>
          <div className={styles.disclaimerCard}>
            ...
            <div className={styles.disclaimerLegal}>
              ...
              <ul>
                <li><strong>Secret professionnel</strong> — Les informations partagées dans cet espace sont couvertes par le secret professionnel (art. L.1110-4 du Code de la santé publique). Tout partage est limité aux professionnels strictement impliqués dans la prise en charge du patient.</li>
                ...
                <li><strong>Finalité légitime</strong> — L&apos;utilisation de cet espace est exclusivement réservée à la coordination des soins et au suivi du patient. Toute utilisation à des fins commerciales, publicitaires ou non liées au soin est interdite.</li>
        ...
        <>
          ...
          <div className={styles.card}>
            ...
            {connectedPros.length === 0 && pendingConns.length === 0 ? (
              <div className={styles.emptySection}>
                Aucun professionnel lié à cet athlète
                ...
                {[...connectedPros, ...pendingConns].map((conn) => (
                  <div key={conn.id} className={styles.proCard}>
                      ...
                      <div className={styles.sharedAccess}>
                        ...
                        <div className={styles.sharedBadges}>
                          {conn.myPermissions.readProgramme && (
                            <button className={styles.sharedBadge} onClick={() => router.push(`/dashboard/coach/programmes?athleteId=${athleteId}`)}>
                          ...
                          {conn.myPermissions.readIndicateurs && (
                            <button className={styles.sharedBadge} onClick={() => router.push(`/dashboard/coach/indicateurs`)}>
                      ...
                      <div className={styles.shareWrapper}>
                        ...
                        {shareMenuConn === conn.id && (
                          <div className={styles.shareMenu}>
                            <button className={styles.shareMenuItem} onClick={() => { router.push(`/dashboard/coach/programmes?athleteId=${athleteId}`); setShareMenuConn(null); }}>
                              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/></svg>
                              Programme
                            </button>
                            <button className={styles.shareMenuItem} onClick={() => { router.push(`/dashboard/coach/indicateurs?athleteId=${athleteId}`); setShareMenuConn(null); }}>
                    ...
                    {conn.status === "connecte" && (
                      <button className={styles.viewSharedBtn} onClick={() => router.push(`/dashboard/coach/reseau/${conn.connectedPro.id}?athleteId=${athleteId}`)}>
                              ...
                              <div className={styles.sharedSection}>
                                ...
                                {sharedData[conn.id].athleteInfo.map((info: any) => (
                                  <div key={info.id} className={styles.sharedPlanCard}>
                                    ...
                                    {info.injuryNote && <div className={styles.sharedPlanMeta}>Note blessure : {info.injuryNote}</div>}
          ...
          <div className={styles.card}>
            ...
            <div className={styles.feedInput}>
              <textarea
                ...
                placeholder="Ajouter une note de collaboration..."
                rows={2}
              />
          ...
          <div className={styles.card}>
            ...
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
              <button className={styles.btnSecondary} onClick={() => router.push(`/dashboard/coach/programmes?athleteId=${athleteId}`)}>
                Partager une séance
              </button>
              <button className={styles.btnSecondary} onClick={() => router.push(`/dashboard/coach/programmes?athleteId=${athleteId}`)}>
                Partager la semaine
              </button>
              <button className={styles.btnSecondary} onClick={() => router.push(`/dashboard/coach/indicateurs`)}>
                Partager un indicateur
              </button>
              <button className={styles.btnSecondary}>Demander validation</button>
              <button className={styles.btnSecondary}>Proposer ajustement</button>
            </div>
          </div>
        </>
      )}
      ...
      {editPermConn && (
        <div className={styles.modalOverlay} onClick={() => setEditPermConn(null)}>
          <div className={styles.modal} onClick={(e) => e.stopPropagation()}>
            ...
            {[
              { key: "readProgramme", label: "Programme (séances)" },
              { key: "readIndicateurs", label: "Indicateurs (tendances)" },
              { key: "readBlessures", label: "Historique blessures" },
              { key: "readDocuments", label: "Documents" },
            ].map((p) => (
            ...
            {[
              { key: "writeNote", label: "Ajouter une note" },
              { key: "writeProgramme", label: "Proposer modification programme" },
              { key: "writeValidation", label: "Valider une séance" },
            ].map((p) => (
              <label key={p.key} style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 8, fontSize: 13, color: "rgba(255,255,255,0.7)", cursor: "pointer" }}>
                <input
                  type="checkbox"
  ...
  return (
    <div className={`${styles.feedItem} ${note.pinned ? styles.feedItemPinned : ""}`}>
      ...
      <div className={styles.feedActions}>
        ...
        <button className={styles.feedActionBtn} onClick={() => router.push("/dashboard/coach/programmes")}>→ Programme</button>
      </div>
    </div>
  );
}
