...
const CATEGORIES = [
  ...
  { value: "pro", label: "Problème avec un athlète" },
...
export default function ProSupportPage() {
  ...
  return (
    <div style={{ maxWidth: "640px", margin: "0 auto", padding: "2rem 1rem", fontFamily: "'Inter', system-ui, sans-serif" }}>
      ...
      <h1 style={{ fontSize: "1.25rem", fontWeight: 700, margin: "0 0 0.25rem", color: "#1e293b" }}>Support Tuatha</h1>
      <p style={{ fontSize: "0.82rem", color: "#64748b", margin: "0 0 1.5rem" }}>
        Besoin d&apos;aide ? Décrivez votre problème ci-dessous.
      </p>
      ...
      {success ? (
        <div style={{ background: "#dcfce7", border: "1px solid #86efac", borderRadius: "12px", padding: "1.5rem", textAlign: "center" }}>
          ...
          <p style={{ fontSize: "0.8rem", color: "#15803d", margin: 0 }}>
            Notre équipe a bien reçu votre message. Réponse sous 24 à 48h ouvrées.
          </p>
        ...
        <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
          ...
          <div>
            <label style={{ display: "block", fontSize: "0.78rem", fontWeight: 600, color: "#334155", marginBottom: "0.3rem" }}>Type de demande</label>
            <select value={category} onChange={e => setCategory(e.target.value)} style={{ width: "100%", padding: "0.6rem 0.75rem", borderRadius: "8px", border: "1px solid #e2e8f0", fontSize: "0.85rem", background: "#fff", color: "#1e293b" }}>
              {CATEGORIES.map(c => <option key={c.value} value={c.value}>{c.label}</option>)}
            </select>
          </div>

          <div>
            <label style={{ display: "block", fontSize: "0.78rem", fontWeight: 600, color: "#334155", marginBottom: "0.3rem" }}>Sujet</label>
            <input value={subject} onChange={e => setSubject(e.target.value)} placeholder="Ex: Erreur lors du paiement" required minLength={3} maxLength={200} style={{ width: "100%", padding: "0.6rem 0.75rem", borderRadius: "8px", border: "1px solid #e2e8f0", fontSize: "0.85rem", color: "#1e293b" }} />
          </div>

          <div>
            <label style={{ display: "block", fontSize: "0.78rem", fontWeight: 600, color: "#334155", marginBottom: "0.3rem" }}>Description</label>
            <textarea value={description} onChange={e => setDescription(e.target.value)} placeholder="Décrivez votre problème en détail..." required minLength={10} maxLength={2000} rows={5} style={{ width: "100%", padding: "0.6rem 0.75rem", borderRadius: "8px", border: "1px solid #e2e8f0", fontSize: "0.85rem", resize: "vertical", color: "#1e293b" }} />
            <div style={{ fontSize: "0.68rem", color: "#94a3b8", marginTop: "0.2rem", textAlign: "right" }}>{description.length}/2000</div>
          </div>

          {error && (
            <div style={{ background: "#fef2f2", border: "1px solid #fecaca", borderRadius: "8px", padding: "0.6rem 1rem", fontSize: "0.8rem", color: "#b91c1c" }}>{error}</div>
          )}

          <button type="submit" disabled={sending} style={{ padding: "0.7rem 1.5rem", borderRadius: "8px", border: "none", background: sending ? "#94a3b8" : "#2563eb", color: "#fff", fontSize: "0.85rem", fontWeight: 600, cursor: sending ? "not-allowed" : "pointer" }}>
            {sending ? "Envoi en cours..." : "Envoyer ma demande"}
          </button>

          <p style={{ fontSize: "0.72rem", color: "#94a3b8", textAlign: "center", margin: 0 }}>
            Urgence sécurité ? Appelez le +33 6.71.63.83.06
          </p>
        </form>
      )}
                ...
                {tickets.map((t: any) => (
                  <div key={t.id} style={{ padding: "0.6rem 0.8rem", border: "1px solid #e2e8f0", borderRadius: "8px", background: "#fafafa" }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                      ...
                      <span style={{ fontSize: "0.65rem", fontWeight: 600, padding: "2px 8px", borderRadius: "9999px", background: `${statusColor(t.status)}15`, color: statusColor(t.status) }}>{statusLabel(t.status)}</span>
                    </div>
