...
const navItems = [
  { label: "Tableau de Bord", href: "/dashboard/medecin", icon: "grid" },
  { label: "Protocoles & Prescriptions", href: "/dashboard/medecin/programmes", icon: "clipboard" },
  { label: "Suivi Médical", href: "/dashboard/medecin/indicateurs", icon: "trending" },
  { label: "Réseau Professionnel", href: "/dashboard/medecin/reseau", icon: "network" },
  { label: "Messagerie", href: "/dashboard/medecin/messagerie", icon: "message" },
  { label: "Documents", href: "/dashboard/medecin/documents", icon: "file" },
  { label: "Cabinet / Équipe", href: "/dashboard/medecin/cabinet", icon: "building" },
  { label: "Facturation et Paiements", href: "/dashboard/medecin/facturation", icon: "wallet" },
];
...
function getBreadcrumb(pathname: string): { label: string; href: string | null }[] {
  ...
  if (item && item.href !== "/dashboard/medecin") {
    crumbs.push({ label: item.label, href: null });
  } else if (pathname.startsWith("/dashboard/medecin/programmes/")) {
    crumbs.push({ label: "Protocoles & Prescriptions", href: "/dashboard/medecin/programmes" });
    crumbs.push({ label: "Détail protocole", href: null });
  }
  return crumbs;
}
                      ...
                      {filteredNotifs.slice(0, 8).map((n) => {
                        ...
                        return (
                          <div key={n.id} className={`${styles.notifItem} ${styles[`notifItem_${level}`]} ${readIds.has(n.id) ? styles.notifItemRead : ""}`} onClick={() => {
                            markAsRead(n.id);
                            if (n.source === "message" || n.source === "athlete_message") { setNotifOpen(false); router.push("/dashboard/medecin/messagerie"); }
            ...
            <div className={styles.userInfo} ref={dropdownRef}>
              ...
              {dropdownOpen && (
                <div className={styles.dropdown}>
                  ...
                  <Link href="/dashboard/medecin/profil" className={styles.dropdownItem} onClick={() => setDropdownOpen(false)}>
