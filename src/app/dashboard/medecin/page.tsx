  ...
  return (
    <div className={styles.page}>
      ...
      <div className={styles.quickAccess}>
        <button className={styles.quickCard} onClick={() => router.push("/dashboard/medecin/indicateurs")}>
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" width="20" height="20"><polyline points="22 12 18 12 15 21 9 3 6 12 2 12"/></svg>
          <span>Suivi médical</span>
        </button>
        <button className={styles.quickCard} onClick={() => router.push("/dashboard/medecin/programmes")}>
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" width="20" height="20"><path d="M16 4h2a2 2 0 0 1 2 2v14a2 2 0 0 1-2 2H6a2 2 0 0 1-2-2V6a2 2 0 0 1 2-2h2"/><rect x="8" y="2" width="8" height="4" rx="1" ry="1"/><path d="M9 12h6"/><path d="M9 16h6"/></svg>
          <span>Protocoles & Prescriptions</span>
        </button>
        <button className={styles.quickCard} onClick={() => router.push("/dashboard/medecin/messagerie")}>
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" width="20" height="20"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg>
          <span>Messagerie</span>
        </button>
        <button className={styles.quickCard} onClick={() => router.push("/dashboard/medecin/documents")}>
      ...
      <div className={styles.filters}>
        <div className={styles.searchWrapper}>
          ...
          <input
            ...
            placeholder="Rechercher par nom, pathologie ou motif..."
            value={search}
        ...
        <div className={styles.emptyState}>
          {search ? (
            ...
            <>
              ...
              <p className={styles.emptyDesc}>Commencez par ajouter votre premier patient pour gérer son suivi médical.</p>
              <div className={styles.emptyChecklist}>
                <div className={styles.checkItem}>✓ Ajoutez un patient avec ses antécédents médicaux</div>
                <div className={styles.checkItem}>✓ Définissez le diagnostic et le protocole de soins</div>
                <div className={styles.checkItem}>✓ Suivez l&apos;évolution clinique au fil du temps</div>
              </div>
              <button className={styles.addBtn} onClick={() => setShowAddModal(true)}>
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><line x1="12" y1="5" x2="12" y2="19" /><line x1="5" y1="12" x2="19" y2="12" /></svg>
                Ajouter un patient
              ...
              {athletes.map((a) => (
                <tr key={a.id} className={styles.row}>
                  ...
                  <td className={styles.injuryCell}>
                    <span className={styles.injuryText}>{a.injuryNote || "Aucun"}</span>
                  </td>
                      ...
                      <div className={styles.menuWrapper}>
                        ...
                        {actionMenu === a.id && (
                          <div ref={actionMenuRef} className={styles.menuDropdown} style={{ position: 'fixed', top: menuPos.top, left: menuPos.left, right: 'auto' }}>
                            ...
                            <div className={styles.menuDivider} />
                            <p className={styles.menuLabel}>État clinique</p>
...
function KanbanBoard({ athletes }: { athletes: Athlete[] }) {
  ...
  const fetchProReminders = useCallback(() => {
    fetch("/api/pro/booking-reminders").then(r => r.json()).then(d => { if (d?.reminders) setProReminders(d.reminders); }).catch(() => {});
  }, []);

  useEffect(() => {
    fetchProReminders();
    const iv = setInterval(fetchProReminders, 30_000);
    return () => clearInterval(iv);
  }, [fetchProReminders]);

  const dismissProReminder = useCallback((id: string) => {
    setProReminders(prev => prev.filter(r => r.id !== id));
    fetch("/api/pro/booking-reminders", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ reminderId: id }) }).catch(() => {});
  }, []);

  // Visio modal state
  const [visioModal, setVisioModal] = useState<{ id: string; title: string; date: string; athleteName: string; visioRoomId: string } | null>(null);
  ...
  // Auto-show visio modal for teleconsult RDVs within 5 min
  useEffect(() => {
    if (visioModal) return;
    const visioTask = tasks.find((t) => {
      if (!t.isRdv || !t.visioRoomId || !t.dueDate) return false;
      if (visioModalDismissed.has(t.id)) return false;
      const diff = new Date(t.dueDate).getTime() - Date.now();
      return diff <= 5 * 60 * 1000 && diff > -30 * 60 * 1000;
    });
    if (visioTask) {
      const athleteName = visioTask.athlete?.name || visioTask.title.split(" — ")[1] || "Patient";
      setVisioModal({
        id: visioTask.id,
        title: visioTask.title,
        date: visioTask.dueDate!,
        athleteName,
        visioRoomId: visioTask.visioRoomId!,
      });
    }
  }, [tasks, visioModal, visioModalDismissed]);

  // Countdown timer
  useEffect(() => {
    if (!visioModal) return;
    const update = () => {
      const diff = new Date(visioModal.date).getTime() - Date.now();
  ...
  const dismissVisioModal = useCallback(() => {
    if (visioModal) setVisioModalDismissed((prev) => new Set(prev).add(visioModal.id));
    setVisioModal(null);
  }, [visioModal]);
  ...
  return (
    ...
    {visioModal && (
      <div className={styles.visioModalOverlay}>
        <div className={styles.visioModalCard}>
          ...
          <div className={styles.visioModalSub}>{visioModal.athleteName}</div>
          <div className={styles.visioModalCountdown}>{visioCountdown}</div>
          <button className={styles.visioModalJoinBtn} onClick={() => { joinVisio(visioModal.visioRoomId); dismissVisioModal(); }}>
    ...
    {/* Pro reminder banners */}
    {proReminders.length > 0 && (
      <div className={styles.proReminderBanners}>
        {proReminders.map((rem) => {
          ...
          const dateStr = evDate.toLocaleDateString("fr-FR", { weekday: "short", day: "numeric", month: "short" });
          const labelMap: Record<string, string> = { j2: "Dans 2 jours", j1: "Demain", h2: "Dans 2h", h1_visio: "Dans 1h", now_visio: "Maintenant" };
          const label = labelMap[rem.type] || "Rappel";
          const colorMap: Record<string, string> = { j2: "#3b82f6", j1: "#f59e0b", h2: "#f47b20", h1_visio: "#10b981", now_visio: "#10b981" };
          const accent = colorMap[rem.type] || "#f47b20";
          const athleteName = rem.athleteUser ? `${rem.athleteUser.prenom} ${rem.athleteUser.nom}` : "Patient";

          return (
            <div key={rem.id} className={`${styles.proReminderCard} ${isNow ? styles.proReminderCardNow : ""}`} style={{ borderColor: `${accent}33` }}>
              <button className={styles.proReminderDismiss} onClick={() => dismissProReminder(rem.id)}>\u00d7</button>
              <span className={styles.proReminderBadge} style={{ background: `${accent}18`, color: accent }}>
                {isNow ? "\u26a1" : "\u23f0"} {label}
              </span>
              <span className={styles.proReminderTitle}>{rem.eventTitle}</span>
              <span className={styles.proReminderMeta}>{athleteName} \u00b7 {dateStr} \u00e0 {heureStr}</span>
              {isVisio && rem.eventVisioRoomId && (
                <button className={styles.kanbanVisioJoin} onClick={() => joinVisio(rem.eventVisioRoomId)}>
              ...
              <div className={styles.kanbanCards}>
                {colTasks.map(task => (
                  ...
                  >
                    ...
                    {task.isRdv && task.visioRoomId && task.column !== "done" && (
                      <button className={styles.kanbanVisioJoin} onClick={(e) => { e.stopPropagation(); joinVisio(task.visioRoomId!); }}>
                        <svg viewBox="0 0 24 24" width="12" height="12" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polygon points="23 7 16 12 23 17 23 7" /><rect x="1" y="5" width="15" height="14" rx="2" ry="2" /></svg>
                        Rejoindre la visio
                      </button>
                    )}
