    ...
    if (!res.ok) {
      ...
      if (err?.consentRequired) {
        ...
        if (ok) {
          ...
          if (granted) {
            // Retry invite after granting consent
        ...
        <div className={styles.headerControls}>
          ...
          {athleteId && (
            <div className={styles.headerBtns}>
              ...
              <button className={styles.btnSecondary} onClick={() => router.push(`/dashboard/medecin/programmes?athleteId=${athleteId}`)}>Voir le protocole</button>
            </div>
          )}
        </div>
      </div>
        ...
        <>
                ...
                {[...connectedPros, ...pendingConns].map((conn) => (
                  <div key={conn.id} className={styles.proCard}>
                      ...
                      <div className={styles.sharedAccess}>
                        ...
                        <div className={styles.sharedBadges}>
                          {conn.myPermissions.readProgramme && (
                            <button className={styles.sharedBadge} onClick={() => router.push(`/dashboard/medecin/programmes?athleteId=${athleteId}`)}>
                          ...
                          {conn.myPermissions.readIndicateurs && (
                            <button className={styles.sharedBadge} onClick={() => router.push(`/dashboard/medecin/indicateurs`)}>
                      ...
                      <div className={styles.shareWrapper}>
                        ...
                        {shareMenuConn === conn.id && (
                          <div className={styles.shareMenu}>
                            <button className={styles.shareMenuItem} onClick={() => { router.push(`/dashboard/medecin/programmes?athleteId=${athleteId}`); setShareMenuConn(null); }}>
                              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/></svg>
                              Protocole
                            </button>
                            <button className={styles.shareMenuItem} onClick={() => { router.push(`/dashboard/medecin/indicateurs`); setShareMenuConn(null); }}>
                    ...
                    {conn.status === "connecte" && (
                      <button className={styles.viewSharedBtn} onClick={() => router.push(`/dashboard/medecin/reseau/${conn.connectedPro.id}?athleteId=${athleteId}`)}>
          ...
          <div className={styles.card}>
            ...
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
              <button className={styles.btnSecondary} onClick={() => router.push(`/dashboard/medecin/programmes?athleteId=${athleteId}`)}>
                Partager un protocole
              </button>
              <button className={styles.btnSecondary} onClick={() => router.push(`/dashboard/medecin/programmes?athleteId=${athleteId}`)}>
                Partager le plan de soins
              </button>
              <button className={styles.btnSecondary} onClick={() => router.push(`/dashboard/medecin/indicateurs`)}>
                Partager un bilan
              </button>
  ...
  return (
    <div className={`${styles.feedItem} ${note.pinned ? styles.feedItemPinned : ""}`}>
      ...
      <div className={styles.feedActions}>
        ...
        <button className={styles.feedActionBtn} onClick={() => router.push("/dashboard/medecin/programmes")}>→ Protocole</button>
      </div>
    </div>
  );
}
