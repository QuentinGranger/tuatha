...
export default function AthleteDashboardPage() {
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    ...
    if (e.key === "ArrowDown") {
      ...
      if (connectedIds.has(selected.id)) {
        router.push(`/dashboard/athlete/pro/${selected.id}`);
      } else {
        handleConnect(selected.id);
      }
  ...
  return (
    <div className={styles.page}>
      <IncomingCallModal />
      <AthleteHeader activeTab="mes-pros" />
        ...
        {hasSearched && results.length > 0 && (
          <div className={styles.resultsList} ref={resultsRef} role="listbox">
            {results.map((pro, idx) => {
              ...
              const isPending = pendingIds.has(pro.id);
              return (
                <div
                  key={pro.id}
                  id={`result-${pro.id}`}
                  className={`${styles.resultCard} ${idx === activeIndex ? styles.resultCardActive : ""} ${isConnected ? styles.resultCardConnected : ""}`}
                  role="option"
                  aria-selected={idx === activeIndex}
                  onMouseEnter={() => setActiveIndex(idx)}
                  onClick={isConnected ? () => router.push(`/dashboard/athlete/pro/${pro.id}`) : undefined}
                >
                  ...
                  <div className={styles.resultActions}>
                    {isConnected ? (
                      ...
                      <button
                        ...
                        onClick={(e) => { e.stopPropagation(); handleConnect(pro.id); }}
                      >
