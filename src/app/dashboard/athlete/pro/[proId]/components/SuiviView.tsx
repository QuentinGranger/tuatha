...
import type { KinePlan, AlertItem, DocItem, PlanExercise, SpecConfig, Tab } from "./types";
import {
  CATEGORIES, catLabel, isYouTubeUrl, toYouTubeEmbed,
  STATUS_LABELS, STATUS_COLORS, formatShort, formatRelative,
} from "./types";
import {
  TabSeances, TabIndicateurs, TabNutriPlan, TabNutriBilan,
  TabMedSuivi, TabMedIndicateurs,
  LogExerciseModal, CreateAlertModal, UploadDocModal,
  AlertsList, HistoryList, DocumentsList,
} from "./index";

// ── SVG Icon helpers ──
const IconFile = <svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z" /><polyline points="14 2 14 8 20 8" /><line x1="16" y1="13" x2="8" y2="13" /><line x1="16" y1="17" x2="8" y2="17" /><polyline points="10 9 9 9 8 9" /></svg>;
const IconPlay = <svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polygon points="5 3 19 12 5 21 5 3" /></svg>;
const IconCal = <svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="4" width="18" height="18" rx="2" ry="2" /><line x1="16" y1="2" x2="16" y2="6" /><line x1="8" y1="2" x2="8" y2="6" /><line x1="3" y1="10" x2="21" y2="10" /></svg>;
const IconBar = <svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M18 20V10" /><path d="M12 20V4" /><path d="M6 20v-6" /></svg>;
const IconCup = <svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M18 8h1a4 4 0 0 1 0 8h-1" /><path d="M2 8h16v9a4 4 0 0 1-4 4H6a4 4 0 0 1-4-4V8z" /><line x1="6" y1="1" x2="6" y2="4" /><line x1="10" y1="1" x2="10" y2="4" /><line x1="14" y1="1" x2="14" y2="4" /></svg>;
const IconPulse = <svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M22 12h-4l-3 9L9 3l-3 9H2" /></svg>;
const IconClip = <svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M9 5H7a2 2 0 0 0-2 2v12a2 2 0 0 0 2 2h10a2 2 0 0 0 2-2V7a2 2 0 0 0-2-2h-2" /><rect x="9" y="3" width="6" height="4" rx="1" /><path d="M9 14l2 2 4-4" /></svg>;
const IconAlert = <svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z" /><line x1="12" y1="9" x2="12" y2="13" /><line x1="12" y1="17" x2="12.01" y2="17" /></svg>;
const IconClock = <svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M12 8v4l3 3" /><circle cx="12" cy="12" r="10" /></svg>;
const IconFolder = <svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ marginRight: 8, verticalAlign: -3 }}><path d="M22 19a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h5l2 3h9a2 2 0 0 1 2 2z" /></svg>;
...
export default function SuiviView({
  proId, specConfig: sc, plans, plansLoading,
  alerts, alertsLoading, docs, docsLoading,
  onRefreshPlans, onRefreshAlerts, onRefreshDocs,
}: SuiviViewProps) {
  const [activeTab, setActiveTab] = useState<Tab>(() => {
    if (sc.hasNutriPlan) return "nutriplan";
    if (sc.hasIndicateurs) return "indicateurs";
    if (sc.hasSeances) return "seances";
    if (sc.hasMedSuivi) return "medsuivi";
    if (sc.hasProgrammes) return "programmes";
    return "suivi";
  });
  ...
  // Collect + deduplicate exercises for video library
  const allExercises: (PlanExercise & { planTitle: string; planId: string })[] = [];
  plans.forEach((p) => p.exercises.forEach((ex) => allExercises.push({ ...ex, planTitle: p.title, planId: p.id })));
  const uniqueVideos = new Map<string, (typeof allExercises)[0]>();
  allExercises.forEach((ex) => { if (!uniqueVideos.has(ex.video.id)) uniqueVideos.set(ex.video.id, ex); });
  ...
  const renderExerciseCard = (ex: PlanExercise, i: number, planId: string, planStatus: string) => (
    <div key={ex.id} className={styles.exCard}>
      <div className={styles.exCardTop}>
        ...
        <div className={styles.exCardActions}>
          <button className={`${styles.exActionBtn} ${styles.exPlayBtn}`} onClick={() => setPlayingVideo(playingVideo === ex.video.id ? null : ex.video.id)}>
            <svg viewBox="0 0 24 24" width="14" height="14" fill="currentColor"><polygon points="5 3 19 12 5 21 5 3" /></svg>
            Vidéo
          </button>
          {planStatus === "active" && (
            <button className={`${styles.exActionBtn} ${styles.exLogBtn}`} onClick={() => setLogExercise({ exerciseId: ex.id, planId, videoTitle: ex.video.title })}>
        ...
        <div className={styles.planCardHeader} onClick={() => setExpandedPlan(isOpen ? null : plan.id)}>
          ...
          {plan.progress > 0 && (
            <div className={styles.planProgress}>
              <div className={styles.planProgressBar}><div className={styles.planProgressFill} style={{ width: `${plan.progress}%` }} /></div>
        ...
        {isOpen && (
          <div className={styles.planBody}>
            <div className={styles.planDetailInfo}>
              {plan.objective && <div className={styles.planInfoRow}><span className={styles.planInfoLabel}>Objectif</span><span>{plan.objective}</span></div>}
              <div className={styles.planInfoRow}><span className={styles.planInfoLabel}>Statut</span><span className={styles.planBadge} style={{ background: `${color}20`, color, borderColor: `${color}40` }}>{label}</span></div>
              {plan.frequency && <div className={styles.planInfoRow}><span className={styles.planInfoLabel}>Fréquence</span><span>{plan.frequency}</span></div>}
              {plan.startDate && <div className={styles.planInfoRow}><span className={styles.planInfoLabel}>Début</span><span>{formatShort(plan.startDate)}</span></div>}
              {plan.endDate && <div className={styles.planInfoRow}><span className={styles.planInfoLabel}>Fin</span><span>{formatShort(plan.endDate)}</span></div>}
              {plan.proName && <div className={styles.planInfoRow}><span className={styles.planInfoLabel}>Prescrit par</span><span>{plan.proName}</span></div>}
            </div>
            {plan.notesPatient && <div className={styles.planInstructions}><strong>{sc.instructionsLabel}</strong><p>{plan.notesPatient}</p></div>}
            {plan.conclusion && <div className={styles.planConclusion}><strong>Conclusion</strong><p>{plan.conclusion}</p></div>}
            <div className={styles.planExSection}>
              ...
              {plan.exercises.length === 0 ? (
                ...
                <div className={styles.planExercises}>
                  {plan.exercises.map((ex, i) => renderExerciseCard(ex, i, plan.id, plan.status))}
                </div>
              )}
            </div>
          </div>
  ...
  // ── Tab config ──
  type TabDef = { key: Tab; label: string; icon: React.ReactNode; show: boolean; badge?: number };
  const tabs: TabDef[] = [
    { key: "programmes", label: sc.tabProgrammes, icon: IconFile, show: sc.hasProgrammes },
    { key: "exercices", label: sc.tabExercices, icon: IconPlay, show: sc.hasExercices },
    { key: "seances", label: sc.tabSeances, icon: IconCal, show: sc.hasSeances },
    { key: "indicateurs", label: "Mes Indicateurs", icon: IconBar, show: sc.hasIndicateurs },
    { key: "nutriplan", label: "Plan Alimentaire", icon: IconCup, show: sc.hasNutriPlan },
    { key: "nutribilan", label: "Bilan Nutri", icon: IconPulse, show: sc.hasNutriPlan },
    { key: "medsuivi", label: "Suivi Médical", icon: IconClip, show: sc.hasMedSuivi },
    { key: "medindicateurs", label: "Indicateurs Médicaux", icon: IconPulse, show: sc.hasMedIndicateurs },
    { key: "suivi", label: "Mon Suivi", icon: IconPulse, show: !sc.hasIndicateurs && !sc.hasNutriPlan && !sc.hasMedSuivi },
    { key: "signalements", label: "Signalements", icon: IconAlert, show: true, badge: openAlertCount || undefined },
    { key: "historique", label: "Historique", icon: IconClock, show: true },
  ];
    ...
    <>
      <div className={styles.container}>
        ...
        <div className={`${styles.tabs} ${sc.hasIndicateurs || sc.hasNutriPlan ? styles.tabsCompact : ""}`}>
          {tabs.filter(t => t.show).map(t => (
            <button key={t.key} className={`${styles.tab} ${activeTab === t.key ? styles.tabActive : ""}`} onClick={() => setActiveTab(t.key)}>
              {t.icon}
              {t.label}
              {t.badge && t.badge > 0 && <span className={styles.tabBadge}>{t.badge}</span>}
            </button>
          ))}
        </div>
        ...
        {activeTab === "programmes" && sc.hasProgrammes && (
          <section className={styles.tabContent}>
            {plansLoading ? (
              <div className={styles.skeleton}><div className={styles.skeletonLine} /><div className={styles.skeletonLine} style={{ width: "70%" }} /><div className={styles.skeletonLine} style={{ width: "50%" }} /></div>
            ) : plans.length === 0 ? (
              ...
              <>
                {activePlans.length > 0 && (
                  <div className={styles.planGroup}><h3 className={styles.planGroupTitle}>Programmes en cours</h3>{activePlans.map((p) => renderPlanCard(p))}</div>
                )}
                {completedPlans.length > 0 && (
                  <div className={styles.planGroup}><h3 className={styles.planGroupTitle}>Programmes terminés</h3>{completedPlans.map((p) => renderPlanCard(p, true))}</div>
                )}
              </>
            )}
          </section>
            ...
            {plansLoading ? (
              ...
              <>
                <div className={styles.videoFilters}>
                  <input className={styles.videoSearchInput} type="text" placeholder="Rechercher une vidéo..." value={videoSearch} onChange={(e) => setVideoSearch(e.target.value)} />
                  <div className={styles.catChips}>
                    <button className={`${styles.catChip} ${!videoCategory ? styles.catChipActive : ""}`} onClick={() => setVideoCategory("")}>Toutes</button>
                    {CATEGORIES.map((c) => (
                      <button key={c.value} className={`${styles.catChip} ${videoCategory === c.value ? styles.catChipActive : ""}`} onClick={() => setVideoCategory(videoCategory === c.value ? "" : c.value)}>{c.label}</button>
                    ))}
                  </div>
                </div>
                {videoList.length === 0 ? (
                      ...
                      <div key={ex.video.id} className={styles.videoCard}>
                        {playingVideo === ex.video.id ? (
                          <div className={styles.videoPlayer}>
                            {isYouTubeUrl(ex.video.url) ? (
                              <iframe src={`${toYouTubeEmbed(ex.video.url)}?autoplay=1`} className={styles.videoElement} allow="autoplay; encrypted-media" allowFullScreen style={{ border: "none" }} />
                            ) : (
                          ...
                          <div className={styles.videoThumb} onClick={() => setPlayingVideo(ex.video.id)}>
                            {ex.video.thumbnail ? <img src={ex.video.thumbnail} alt={ex.video.title} /> : (
                              <div className={styles.videoThumbPlaceholder}><svg viewBox="0 0 24 24" width="28" height="28" fill="none" stroke="currentColor" strokeWidth="1.5"><polygon points="5 3 19 12 5 21 5 3" /></svg></div>
                            )}
                            <div className={styles.videoPlayBtn}><svg viewBox="0 0 24 24" width="24" height="24" fill="#fff"><polygon points="5 3 19 12 5 21 5 3" /></svg></div>
                            {ex.video.duration && <span className={styles.videoDuration}>{Math.floor(ex.video.duration / 60)}:{(ex.video.duration % 60).toString().padStart(2, "0")}</span>}
                          </div>
                        )}
                        ...
                        <div className={styles.videoCardFooter}>
                          <button className={styles.videoWatchBtn} onClick={() => setPlayingVideo(playingVideo === ex.video.id ? null : ex.video.id)}>
                            <svg viewBox="0 0 24 24" width="14" height="14" fill="currentColor"><polygon points="5 3 19 12 5 21 5 3" /></svg>
                    ...
                    <div key={plan.id} className={styles.suiviCard}>
                      <div className={styles.suiviCardHeader}>
                        ...
                        <div className={styles.suiviProgress}>
                          ...
                          <div className={styles.suiviProgressBar}><div className={styles.suiviProgressFill} style={{ width: `${plan.progress}%` }} /></div>
                        </div>
                      </div>
                      ...
                      {plan.nextRdvDate && (
                        <div className={styles.suiviRdv}>
                          ...
                          <div>
                            ...
                            <span className={styles.suiviRdvValue}>{formatShort(plan.nextRdvDate)}{plan.nextRdvTime ? ` à ${plan.nextRdvTime}` : ""}</span>
                            {plan.nextRdvLocation && <span className={styles.suiviRdvLoc}>{plan.nextRdvLocation}</span>}
                          </div>
                        </div>
                      )}
                      {plan.notesPatient && <div className={styles.suiviNotes}><strong>{sc.instructionsLabel}</strong><p>{plan.notesPatient}</p></div>}
                      {plan.exercises.length > 0 && (
        ...
        {activeTab === "signalements" && (
          <section className={styles.tabContent}>
            <div className={styles.suiviSectionHeader}>
              ...
              <button className={styles.suiviBtnPrimary} onClick={() => setShowCreateAlert(true)}>+ Signaler un problème</button>
            </div>
            {alertsLoading ? (
              <div className={styles.skeleton}><div className={styles.skeletonLine} /><div className={styles.skeletonLine} style={{ width: "60%" }} /></div>
            ) : alerts.length === 0 ? (
        ...
        {activeTab === "historique" && (
          <section className={styles.tabContent}>
            {plansLoading ? (
              <div className={styles.skeleton}><div className={styles.skeletonLine} /><div className={styles.skeletonLine} style={{ width: "50%" }} /></div>
            ) : completedPlans.length === 0 ? (
              <div className={styles.tabEmpty}>
      ...
      <section className={styles.docSection}>
        <div className={styles.suiviSectionHeader} style={{ marginBottom: 16 }}>
          <h3 className={styles.suiviSectionTitle}>{IconFolder} Documents partagés</h3>
      ...
      {/* ── Modals ── */}
      {showUploadDoc && <UploadDocModal proId={proId} onClose={() => setShowUploadDoc(false)} onUploaded={() => { setShowUploadDoc(false); onRefreshDocs(); }} />}
      {logExercise && <LogExerciseModal exerciseId={logExercise.exerciseId} planId={logExercise.planId} videoTitle={logExercise.videoTitle} onClose={() => setLogExercise(null)} onLogged={() => { setLogExercise(null); onRefreshPlans(); }} />}
      {showCreateAlert && <CreateAlertModal proId={proId} plans={activePlans} onClose={() => setShowCreateAlert(false)} onCreated={() => { setShowCreateAlert(false); onRefreshAlerts(); }} />}
    </>
  );
}
