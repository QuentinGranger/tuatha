...
export default function MessageriePage() {
  ...
  const selectConversation = (conv: Conversation) => {
    ...
    // Determine target key and restore draft
  ...
  const stopAndSendRecording = async () => {
    ...
    const optimisticMsg: Message = {
      ...
      attachments: [{
        ...
        filename, originalName: filename, mimeType: file.type, size: file.size, filePath: "",
      }],
  ...
  const retryMessage = async (tempId: string) => {
    ...
    try {
      const res = await offlineFetch("/api/reseau/messages", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ receiverProId: selectedPro.id, content: msg.content || undefined, replyToId: msg.replyToId }),
      });
  ...
  const sendMessage = async () => {
    ...
    const optimisticMsg: Message = {
      ...
      attachments: filesToSend.length > 0 ? filesToSend.map(f => ({
        id: `_att_${Math.random().toString(36).slice(2)}`,
        filename: f.name, originalName: f.name, mimeType: f.type, size: f.size, filePath: "",
      })) : undefined,
      createdAt: new Date().toISOString(),
      _status: "sending",
      _tempId: tempId,
    ...
    try {
      ...
      const postBody: any = { content: content || undefined, replyToId: rId, attachments };
      if (selectedGroup) { postBody.conversationId = selectedGroup.id; }
      else if (selectedPro) { postBody.receiverProId = selectedPro.id; }
  ...
  return (
    <div className={styles.container}>
      ...
      <div className={styles.sidebar}>
        ...
        <div className={styles.convList}>
          ...
          {filtered.map((conv) => {
            ...
            return (
            <div
              key={conv.isGroup ? conv.groupId : conv.proId}
            ...
            >
              <div className={`${styles.convAvatar} ${conv.isGroup ? styles.groupAvatar : ""}`}>
                ...
                ) : fixAvatar(conv.pro?.avatarPath ?? null) ? (
                  <img src={fixAvatar(conv.pro?.avatarPath ?? null)!} alt="" className={styles.convAvatarImg} />
                ) : (
              ...
              <div className={styles.convInfo}>
                <div className={styles.convInfoTop}>
                  <span className={styles.convName}>{conv.isGroup ? (conv.name || "Groupe") : `${conv.pro?.prenom ?? ""} ${conv.pro?.nom ?? ""}`}</span>
            ...
            return (
              ...
              >
                <div className={styles.convAvatar} style={{ background: "rgba(76, 175, 80, 0.12)", color: "#4caf50" }}>
                  {ac.athlete.avatarUrl ? (
                    <img src={ac.athlete.avatarUrl} alt="" className={styles.convAvatarImg} />
                  ) : initials}
                </div>
                <div className={styles.convInfo}>
                  <div className={styles.convInfoTop}>
                    <span className={styles.convName}>
                      {ac.athlete.name}
                      <span style={{ fontSize: 9, color: "rgba(255,255,255,0.3)", marginLeft: 6, fontWeight: 400 }}>Patient</span>
                    </span>
                  ...
                  <div className={styles.convInfoBottom}>
                    <span className={`${styles.convPreview} ${ac.unread > 0 ? styles.convPreviewUnread : ""}`}>
                      ...
                      {ac.lastMessage?.content ?? "Nouveau"}
                    </span>
                ...
                {filteredAthletes.map((a: any) => (
                  <div key={a.id} className={styles.contactItem} onClick={() => {
                      ...
                      .then(d => {
                        if (d.messages && Array.isArray(d.messages)) {
                          setMessages(d.messages.map((m: any) => ({
                            ...
                            senderPro: m.senderType === "pro"
                              ? { id: myProId, nom: "", prenom: "Moi", specialite: null, avatarPath: null }
                              : { id: m.athleteUserId, nom: a.name.split(" ").pop() || "", prenom: a.name.split(" ")[0] || "", specialite: null, avatarPath: null },
                            reactions: [], pinned: false, important: false, replyToId: null,
                          })));
                        }
                      })
                    ...
                    <div className={styles.contactAvatarAthlete}>
                      {a.avatarUrl ? <img src={a.avatarUrl} alt="" style={{ width: "100%", height: "100%", objectFit: "cover", borderRadius: "50%" }} /> : a.name.split(" ").map((w: string) => w[0]).join("").toUpperCase().slice(0, 2)}
                    </div>
                    <div className={styles.contactInfo}>
                ...
                <>
                  <div className={styles.chatHeaderAvatar} style={{ background: "rgba(76, 175, 80, 0.12)", color: "#4caf50" }}>
                    {selectedAthlete.avatarUrl ? (
                      <img src={selectedAthlete.avatarUrl} alt="" style={{ width: 42, height: 42, borderRadius: "50%", objectFit: "cover" }} />
                    ) : (
                      selectedAthlete.name.split(" ").map(w => w[0]).join("").toUpperCase().slice(0, 2)
                    )}
                  </div>
                  <div className={styles.chatHeaderInfo}>
      ...
      {showGroupModal && (
        <div className={styles.groupModalOverlay} onClick={() => setShowGroupModal(false)}>
          <div className={styles.groupModal} onClick={e => e.stopPropagation()}>
            <h3>Nouveau groupe</h3>
            <input className={styles.groupModalInput} placeholder="Nom du groupe (optionnel)" value={groupName} onChange={e => setGroupName(e.target.value)} />
            <div className={styles.groupModalLabel}>Membres ({groupMemberIds.length} sélectionnés, min. 2)</div>
            <div className={styles.groupModalMembers}>
              {contacts.professionals.map((p: any) => (
                <label key={p.id} className={styles.groupModalMember}>
                  <input type="checkbox" checked={groupMemberIds.includes(p.id)} onChange={() => setGroupMemberIds(prev => prev.includes(p.id) ? prev.filter((id: string) => id !== p.id) : [...prev, p.id])} />
                  <span>{p.prenom} {p.nom}</span>
                  {p.specialite && <span className={styles.groupModalSpec}>{p.specialite}</span>}
                </label>
              ))}
            </div>
            <div className={styles.groupModalActions}>
              <button className={styles.groupModalCancel} onClick={() => setShowGroupModal(false)}>Annuler</button>
              <button className={styles.groupModalCreate} disabled={groupMemberIds.length < 2} onClick={createGroup}>Créer</button>
            </div>
          </div>
        </div>
      )}
