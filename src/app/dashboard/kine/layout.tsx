...
const navItems = [
  { label: "Tableau de Bord", href: "/dashboard/kine", icon: "grid" },
  { label: "Séances & Protocoles", href: "/dashboard/kine/programmes", icon: "clipboard" },
  { label: "Bilans & Suivi", href: "/dashboard/kine/indicateurs", icon: "trending" },
  { label: "Réseau Professionnel", href: "/dashboard/kine/reseau", icon: "network" },
  { label: "Messagerie", href: "/dashboard/kine/messagerie", icon: "message" },
  { label: "Documents", href: "/dashboard/kine/documents", icon: "file" },
  { label: "Cabinet / Équipe", href: "/dashboard/kine/cabinet", icon: "building" },
  { label: "Facturation et Paiements", href: "/dashboard/kine/facturation", icon: "wallet" },
];
...
function getBreadcrumb(pathname: string): { label: string; href: string | null }[] {
  const crumbs: { label: string; href: string | null }[] = [
    { label: "Dashboard", href: "/dashboard/kine" },
  ];
  const item = navItems.find((n) => n.href === pathname);
  if (item && item.href !== "/dashboard/kine") {
    crumbs.push({ label: item.label, href: null });
  } else if (pathname.startsWith("/dashboard/kine/programmes/")) {
    crumbs.push({ label: "Séances & Protocoles", href: "/dashboard/kine/programmes" });
    crumbs.push({ label: "Détail séance", href: null });
  }
  return crumbs;
}

export default function KineDashboardLayout({
  children,
}: {
  ...
  const [connectToast, setConnectToast] = useState<{ message: string; type: "success" | "info" | "error" } | null>(null);
  const connectToastTimer = useRef<ReturnType<typeof setTimeout> | null>(null);

  const showConnectToast = (message: string, type: "success" | "info" | "error" = "success") => {
    if (connectToastTimer.current) clearTimeout(connectToastTimer.current);
    setConnectToast({ message, type });
    connectToastTimer.current = setTimeout(() => setConnectToast(null), 3500);
  };
  ...
  // Track which accepted connections we've already toasted
  const seenAcceptedRef = useRef<Set<string>>(new Set());
  ...
  useSSE<any[]>({
    ...
    onMessage: (data) => {
      ...
      // Detect new connection_accepted notifications → toast + refresh event
      for (const n of data) {
        if (n.source === "connection_accepted" && !seenAcceptedRef.current.has(n.id)) {
          seenAcceptedRef.current.add(n.id);
          const name = n.meta?.athletePrenom && n.meta?.athleteNom
            ? `${n.meta.athletePrenom} ${n.meta.athleteNom}`
            : "L'athlète";
          showConnectToast(`${name} a accepté votre demande !`, "success");
          // Dispatch event so child pages (dashboard, etc.) can refresh their athlete list
          window.dispatchEvent(new CustomEvent("tuatha:connection-accepted", { detail: n.meta }));
        }
      }
    },
  });
  ...
  const connectAthlete = async (athleteUserId: string) => {
    const athlete = searchResults.find((a) => a.id === athleteUserId);
    const athleteName = athlete ? `${athlete.prenom} ${athlete.nom}` : "l'athlète";
    try {
      ...
      const data = await res.json();
      if (res.ok) {
        setSearchResults((prev) => prev.map((a) => a.id === athleteUserId ? { ...a, connectionStatus: "pending" } : a));
        showConnectToast(`Demande envoyée à ${athleteName}`, "success");
      } else if (res.status === 409) {
        setSearchResults((prev) => prev.map((a) => a.id === athleteUserId ? { ...a, connectionStatus: data.status || "pending" } : a));
        showConnectToast(data.status === "accepted" ? `Déjà connecté avec ${athleteName}` : `Demande déjà en attente pour ${athleteName}`, "info");
      } else {
        showConnectToast(data.error || "Erreur lors de l'envoi", "error");
      }
    } catch {
      showConnectToast("Erreur réseau, réessayez.", "error");
    }
  };
  ...
  return (
    <div className={styles.layout}>
                      ...
                      {filteredNotifs.slice(0, 8).map((n) => {
                        ...
                        return (
                          <div key={n.id} className={`${styles.notifItem} ${styles[`notifItem_${level}`]} ${readIds.has(n.id) ? styles.notifItemRead : ""}`} onClick={() => {
                            markAsRead(n.id);
                            if (n.source === "message" || n.source === "athlete_message") { setNotifOpen(false); router.push("/dashboard/kine/messagerie"); }
                            else if (n.source === "connection_accepted") { setNotifOpen(false); router.push("/dashboard/kine"); }
            ...
            <div className={styles.userInfo} ref={dropdownRef}>
              ...
              {dropdownOpen && (
                <div className={styles.dropdown}>
                  ...
                  <Link href="/dashboard/kine/profil" className={styles.dropdownItem} onClick={() => setDropdownOpen(false)}>
      ...
      {/* Connect toast */}
      {connectToast && (
        <div className={styles.connectToast} data-type={connectToast.type}>
          {connectToast.type === "success" && (
          ...
          {connectToast.type === "info" && (
            <svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10" /><path d="M12 16v-4" /><path d="M12 8h.01" /></svg>
          )}
          {connectToast.type === "error" && (
