...
// GET /api/notifications — calendar events + kanban tasks with reminders due
export const GET = withAuth(async (_request, ctx) => {
  try {
    ...
    const events = await prisma.calendarEvent.findMany({
      where: { professionnelId: pro.id, reminderMinutes: { not: null }, reminderSeen: false },
    ...
    const dueEvents = events
      ...
      .map((ev) => ({ ...ev, source: "event" as const }));

    // Kanban tasks with dueDate + reminder due
    const tasks = await prisma.kanbanTask.findMany({
      where: { professionnelId: pro.id, reminderMinutes: { not: null }, reminderSeen: false, dueDate: { not: null } },
      include: { athlete: { select: { id: true, name: true } } },
      orderBy: { dueDate: "asc" },
    });
    ...
    const dueTasks = tasks
      ...
      .map((t) => ({ id: t.id, title: t.title, date: t.dueDate!, type: "task", color: t.priority === "high" ? "red" : t.priority === "medium" ? "orange" : "gray", athlete: t.athlete, source: "kanban" as const }));

    // Kanban tasks overdue (past dueDate, not in "done" column, no reminder needed)
    const overdue = await prisma.kanbanTask.findMany({
      where: { professionnelId: pro.id, dueDate: { lt: now }, column: { not: "done" }, reminderSeen: false, reminderMinutes: null },
      include: { athlete: { select: { id: true, name: true } } },
    });

    const overdueNotifs = overdue.map((t) => ({ id: t.id, title: `⏰ ${t.title}`, date: t.dueDate!, type: "task", color: "red", athlete: t.athlete, source: "kanban" as const }));

    // Pending invitations received (where email matches this pro's email)
    const proRecord = await (prisma as any).professionnel.findUnique({ where: { id: pro.id }, select: { email: true } });
    ...
    try {
      const unreadMessages = await (prisma as any).proMessage.findMany({
        where: { receiverProId: pro.id, read: false },
        include: {
    ...
    try {
      const unreadAthleteMessages = await (prisma as any).athleteProMessage.findMany({
        where: { professionnelId: pro.id, senderType: "athlete", read: false },
        include: {
          athleteUser: { select: { id: true, nom: true, prenom: true, sport: true, avatarPath: true } },
        },
        orderBy: { createdAt: "desc" },
      });
      ...
      athleteMessageNotifs = Array.from(byAthlete.values()).map((g) => ({
        ...
        type: "message",
        color: "green",
        athlete: { name: `${g.athlete.prenom} ${g.athlete.nom}` },
    ...
    } catch (_) { /* athleteProMessage table might not exist yet */ }

    // Security alerts (last 7 days, max 20)
    ...
    try {
      ...
      const visioEvents = await (prisma as any).calendarEvent.findMany({
        where: { professionnelId: pro.id, visioRoomId: { not: null }, deletedAt: null },
    ...
    // Connection requests from athletes (pending)
    let connectionNotifs: any[] = [];
    try {
      const pendingConnections = await (prisma as any).connectionRequest.findMany({
        where: { professionnelId: pro.id, status: "pending", requestedBy: "athlete" },
        include: {
    ...
    // Recently accepted connections (pro-initiated, accepted by athlete in last 2 min)
    ...
    // Consultation prep completed by athlete (upcoming events, last 7 days)
    ...
    try {
      ...
      const completedPreps = await (prisma as any).consultationPrep.findMany({
        where: {
          ...
          calendarEvent: { professionnelId: pro.id, deletedAt: null },
        },
        include: {
    ...
    try {
      const nutriAlerts = await (prisma as any).nutriAlert.findMany({
        where: {
          status: "unread",
          athlete: { professionnelId: pro.id },
        },
        include: {
          athlete: { select: { id: true, name: true } },
        },
    ...
    try {
      // Find groups where this pro is a member
