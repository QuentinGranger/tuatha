...
// ─── Fetch all notifications for an athlete (mirrors the polling endpoint) ───

async function getNotificationsForAthlete(athleteUserId: string) {
  // Unread pro→athlete messages grouped by pro
  const unreadMessages = await prisma.athleteProMessage.findMany({
    where: { athleteUserId, senderType: "pro", read: false },
    include: {
  ...
  const messageNotifs = Array.from(byPro.values()).map((g) => ({
    id: `msg-${g.pro.id}`,
  ...
  try {
    const healthConnections = await prisma.healthAppConnection.findMany({
      where: { athleteUserId, status: "connected" },
  ...
  try {
    const pendingRequests = await prisma.connectionRequest.findMany({
      where: { athleteUserId, status: "pending", requestedBy: "pro" },
      include: {
        professionnel: { select: { id: true, nom: true, prenom: true, specialite: true, avatarPath: true } },
      },
      orderBy: { createdAt: "desc" },
    ...
    connectionNotifs = pendingRequests.map((c: any) => ({
      ...
      subtitle: c.professionnel.specialite || "Professionnel de santé",
      date: c.createdAt,
  ...
  // Coach sessions planned (visibleAthlete, last 7 days) + feedback requested
  ...
  try {
    ...
    const pendingAccess = await (prisma as any).dataAccessRequest.findMany({
      where: { athleteUserId, status: "pending" },
      include: {
        professionnel: { select: { id: true, nom: true, prenom: true, specialite: true, avatarPath: true } },
      },
      orderBy: { createdAt: "desc" },
  ...
  try {
    ...
    const in24h = new Date(nowCP.getTime() + 24 * 60 * 60 * 1000);

    const upcomingEvents = await prisma.calendarEvent.findMany({
      where: {
        athleteUserId,
        deletedAt: null,
        date: { gt: nowCP, lte: in24h },
      },
      select: {
  ...
  const stream = new ReadableStream({
    start(controller) {
      ...
      const poll = async () => {
        if (!alive) return;
        try {
          const data = await getNotificationsForAthlete(athleteUserId);
          ...
          console.error("[SSE athlete notifications] poll error:", err);
        }
      };
